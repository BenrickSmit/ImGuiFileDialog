...
static void ClickedOnEntrySelect(size_t id, bool isSel, bool isFolder) {
  ...
  if (ImGui::GetIO().KeyShift == ImGui::GetIO().KeyCtrl ||
      ImGui::IsMouseClicked(ImGuiMouseButton_Right)) {  // both on or both off => standard select
    if (isSel) {
      ...
      for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; row++) {
        ...
        for (size_t col = 0; col < itemsInThisLine; col++) {
          ...
          if (ImGui::IsItemClicked() ||
              ImGui::IsItemClicked(
                  ImGuiMouseButton_Right)) {  // directly using return value of Selectable doesnt
            ...
            if (ImGui::IsItemHovered()) {
              ...
              if (fileData->thumbnail->origWidth != -1 &&
                  fileData->thumbnail->origHeight !=
                      -1) {  // check if actual (non thumbnail scaled) size was entered
                ImGui::PushStyleColor(ImGuiCol_Text, settings.descTextCol);
                ImGui::Separator();
