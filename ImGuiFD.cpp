...
namespace ImGuiFD
{
  namespace utils
  {
    ...
    const char *getFileName(const char *path, const char *path_end = 0)
    {
      ...
      const char *lastDiv =
          ImMax(lastSlash != nullptr ? lastSlash : 0, lastBSlash != nullptr ? lastBSlash : 0);
    ...
    bool InputTextString(const char *label,
                         const char *hint,
                         ds::string *str,
                         ImGuiInputTextFlags flags = 0,
                         const ImVec2 &size = {0, 0})
    {
      return ImGui::InputTextEx(label,
                                hint,
                                (char *)str->c_str(),
                                (int)str->capacity(),
                                size,
  ...
  static int compareSortSpecs(const void *lhs, const void *rhs)
  {
    ...
    for (int i = 0; i < globalSortSpecs->SpecsCount; i++)
    {
      ...
      {
        int64_t diff =
            ((int64_t)a.creationDate - (int64_t)b.creationDate); // int64 to prevent overflows
        if (diff < 0)
          delta = -1;
        if (diff > 0)
          delta = 1;
      ...
      {
        int64_t diff =
            ((int64_t)a.lastModified - (int64_t)b.lastModified); // int64 to prevent overflows
        if (diff < 0)
          delta = -1;
        if (diff > 0)
          delta = 1;
  ...
  public:
    ...
    {
      ds::string rawPathFix = utils::fixDirStr(rawPath)
                                  .c_str(); // fixes weird behaviour on win when e.g. setting to "D:"
      parts = utils::splitPath(
          utils::fixDirStr(Native::getAbsolutePath(rawPathFix.c_str()).c_str()).c_str());
    }
        ...
        bool passes(const char *str)
        {
...

          {
            ...
            else
            {
              ...
              if (fileName.size() > 0 &&
                  strncmp(fileName.c_str(),
                          str,
                          fileName.size() < (size_t)(dotPos - str) ? fileName.size() : (dotPos - str)) !=
                      0)
                return false;
            }
          }
  ...
  public:
    ...
    FileDialog(ImGuiID id,
               const char *str_id,
               const char *filter,
               const char *path,
               ImGuiFDMode mode,
               ImGuiFDDialogFlags flags = 0,
               size_t maxSelections = 1)
        : str_id(str_id),
          id(id),
          path(utils::fixDirStr(Native::getAbsolutePath(path).c_str())),
          currentPath(this->path.c_str()),
          oldPath(this->path),
          undoStack(32),
          redoStack(32),
          entrys(filter),
          mode(mode),
          isModal((flags & ImGuiFDDialogFlags_Modal) != 0),
    ...
    void setInputTextToSelected()
    {
      if (selected.size() == 0)
      {
        ...
        {
          inputText = ds::string("\"") + currentPath.toString() +
                      "\""; // if selecting a directory, always have the folder we are in as default
  ...
  static ImRect TextWrappedCentered(const char *text, float maxWidth, int maxLines = -1)
  {
    ...
    {
      const char *wrap =
          g.Font->CalcWordWrapPositionA(g.FontSize / g.Font->FontSize, s, text + len, maxWidth);
      ...
      {
        const ImVec2 cursorPos = cursorStart + ImVec2{maxWidth / 2 - lineSize.x / 2,
                                                      ImGui::GetTextLineHeightWithSpacing() * lineInd};
        if (cursorPos.x < minPos.x)
          minPos.x = cursorPos.x;
        if (cursorPos.y < minPos.y)
          minPos.y = cursorPos.y;

        if ((cursorPos + lineSize).x > maxPos.x)
          maxPos.x = (cursorPos + lineSize).x;
        if ((cursorPos + lineSize).y > maxPos.y)
          maxPos.y = (cursorPos + lineSize).y;
        ...
        if (lineInd + 1 >= maxLines && wrap < text + len)
        {
          ImGui::RenderTextEllipsis(ImGui::GetWindowDrawList(),
                                    ...
                                    ImGui::GetCursorScreenPos() + lineSize + ImVec2{0, g.Style.FramePadding.y},
                                    ImGui::GetCursorScreenPos().x + maxWidth,
                                    ImGui::GetCursorScreenPos().x + maxWidth,
                                    s,
                                    text + len,
                                    0);
  ...
  static bool ComboVertical(const char *str_id,
                            size_t *v,
                            const char **labels,
                            size_t labelCnt,
                            const ImVec2 &size_arg = {0, 0})
  {
    ...
    ImVec2 size = ImVec2(size_arg.x != 0 ? size_arg.x : avail.x,
                         size_arg.y != 0 ? size_arg.y : ImGui::GetFrameHeight());

    ImVec2 borderPad = {1, 1};
    ImRect rec(
        ImGui::GetCursorScreenPos() - borderPad, ImGui::GetCursorScreenPos() + size + borderPad);
    ImGui::GetWindowDrawList()->AddRectFilled(
        rec.Min, rec.Max, ImColor(ImGui::GetStyleColorVec4(ImGuiCol_ChildBg)));
    ...
    ImGui::GetWindowDrawList()->AddRect(
        rec.Min, rec.Max, ImColor(ImGui::GetStyleColorVec4(ImGuiCol_Border)));
  ...
  static void ClickedOnEntrySelect(size_t id, bool isSel, bool isFolder)
  {
    ...
    { // both on or both off => standard select
      if (isSel)
      {
  ...
  {
    ImGui::Checkbox("Show directory first", &settings.showDirFirst);
    ...
    if (ImGui::ColorEdit4("Icon text color", (float *)&settings.iconTextCol))
    {
      settings.iconTextColSetByUser = true;
    }
  ...
  static void DrawDirBar()
  {
    ...
    const ImRect rec(ImGui::GetCursorScreenPos(),
                     ImGui::GetCursorScreenPos() + ImVec2(width, ImGui::GetFrameHeight()));
    ...
    ImGui::BeginChild("Directory Bar", ImVec2(width, ImGui::GetFrameHeight()));
    ...
    if (!fd->isEditingPath)
    {
      ...
      // draw Background
      ImGui::GetWindowDrawList()->AddRectFilled(
          recOuter.Min, recOuter.Max, ImColor(ImGui::GetStyleColorVec4(ImGuiCol_TableHeaderBg)));
      ...
      if (!fd->forceDisplayAllDirs)
      {
        for (ptrdiff_t i = fd->currentPath.parts.size() - 1; i >= 0; i--)
        {
          const float label_size =
              ImGui::CalcTextSize(fd->currentPath.parts[i].c_str(), NULL, true).x;
          float nextWidth =
              totalWidth + (label_size + style.FramePadding.x * 2.0f) + style.ItemSpacing.x;
      ...
      // ImGui::GetWindowDrawList()->AddRect(ImGui::GetCursorScreenPos(), ImGui::GetCursorScreenPos()
      // + ImVec2(totalWidth, ImGui::GetTextLineHeight()), IM_COL32(255, 0, 0, 255));

      bool doesntFit = lastToFit != (size_t)-1;

      size_t startOn = 0;
      const float ellipseBtnWidth =
    ...
    else
    {
      ...
      if (utils::InputTextString("##editPath",
                                 "Enter a Directory",
                                 &fd->editOnPathStr,
    ...
    // draw border
    ImGui::GetWindowDrawList()->AddRect(
        recOuter.Min, recOuter.Max, ImColor(ImGui::GetStyleColorVec4(ImGuiCol_Border)));
  }
  static void DrawNavigation()
  {
    ...
    fd->entrys.drawSeachBar(ImGui::GetContentRegionAvail().x -
                            (displayModeBtnWidth + style.ItemSpacing.x) -
