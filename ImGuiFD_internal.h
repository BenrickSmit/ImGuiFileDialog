...
namespace ds
{
  ...
  public:
    ...
    inline size_t index_from_ptr(const T *it) const
    {
      IM_ASSERT(it >= Data && it < Data + Size);
      const ptrdiff_t off = it - Data;
      return (size_t)off;
  ...
  // find value, if not found return (size_t)-1; compare needs to be a function like object with
  // (const T& a, size_t ind_of_b) -> int
  ...
  // find value, if not found return where to insert it; compare needs to be a function like object
  // with (const T& a, size_t ind_of_b) -> int
  template <typename T, typename CMP>
...
namespace ImGuiFD
{
  struct FDInstance
  {
    ...
    void OpenDialog(ImGuiFDMode mode,
                    const char *path,
